...
def setup_env(env):
    ...
    if platform.system() == "Linux":
        ...
        logging.info(f"++ rpp tests only supported on Linux")
        sys.exit(0)
...
def execute_tests(env):
    ...
    # rpp tests are shipped as CMake source and must be built on the target
    # machine. This serves two purposes:
    # 1. Verifies that the installed rpp headers and libraries are functional.
    ...
    if int(match.group(1)) == 0:
        raise RuntimeError("CTest discovered zero rpp tests")
